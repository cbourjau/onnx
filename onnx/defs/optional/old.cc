...
ONNX_OPERATOR_SET_SCHEMA(
    OptionalHasElement,
    15,
    OpSchema().FillUsing(OptionalHasElement_v15_FillSpec).TypeAndShapeInferenceFunction([](InferenceContext& ctx) {
...
ONNX_OPERATOR_SET_SCHEMA(
    OptionalGetElement,
    15,
    OpSchema().FillUsing(OptionalGetElement_v15_FillSpec).TypeAndShapeInferenceFunction([](InferenceContext& ctx) {
      const size_t numInputs = ctx.getNumInputs();
      if (numInputs != 1) {
