...
ONNX_OPERATOR_SET_SCHEMA(
    Optional,
    15,
    OpSchema().FillUsing(Optional_v15_FillSpec).TypeAndShapeInferenceFunction([](InferenceContext& ctx) {
...
ONNX_OPERATOR_SET_SCHEMA(
    OptionalHasElement,
    18,
    OpSchema().FillUsing(OptionalHasElement_v18_FillSpec).TypeAndShapeInferenceFunction([](InferenceContext& ctx) {
      const size_t numInputs = ctx.getNumInputs();
...
ONNX_OPERATOR_SET_SCHEMA(
    OptionalGetElement,
    18,
    OpSchema().FillUsing(OptionalGetElement_v18_FillSpec).TypeAndShapeInferenceFunction([](InferenceContext& ctx) {
      const size_t numInputs = ctx.getNumInputs();
      if (numInputs != 1) {
