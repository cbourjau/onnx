---
inherit: v12/softmaxcrossentropyloss
since_version: 13
type_constraints:
  update:
  - name: T
...
inputs:
  ...
  - name: labels
    type: Tind
    doc: The ground truth output tensor, with shape [batch_size], or [batch_size,
  ...
  - name: weights
    ...
    doc: A manual rescaling weight given to each class. If given, it has to be a 1D
      Tensor assigning weight to each of the classes. Otherwise, it is treated as
      if having all ones.
    kind: Option
    differentiable: false
outputs:
  ...
  - name: output
    type: T
    doc: Weighted loss float Tensor. If reduction is 'none', this has the shape of
      [batch_size], or [batch_size, D1, D2, ..., Dk] in case of K-dimensional loss.
