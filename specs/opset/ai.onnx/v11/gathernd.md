...
- name: indices
  type: tensor(int64)
  doc: Tensor of rank q >= 1. All index values are expected to be within bounds [-s,
    s-1] along axis of size s. It is an error if any of the index values are out of
    bounds.
outputs:
- name: output
  type: T
