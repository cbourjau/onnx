...
- name: sequence_lens
  type: I
  doc: Optional tensor specifying lengths of the sequences in a batch. If this input
    is not specified, all sequences are assumed to be of the maximum sequence length
    (the dimension of the sequence axis of the scan_input tensors).
  kind: Option
- name: initial_state_and_scan_inputs
  type: V
  doc: Initial values of the loop's N state variables followed by M scan_inputs
  kind:
    homogeneous: false
    min_arity: 1
outputs:
- name: final_state_and_scan_outputs
  type: V
  doc: Final values of the loop's N state variables followed by K scan_outputs
  kind:
    homogeneous: false
    min_arity: 1
attributes:
- name: body
  type: graph
  doc: 'The graph run each iteration. It has N+M inputs: (loop state variables...,
...
- name: num_scan_inputs
  type: int
  doc: 'An attribute specifying the number of scan_inputs M. '
  required: true
- name: directions
  type: ints
  doc: 'An optional list of M flags. The i-th element of the list specifies the direction
    to be scanned for the i-th scan_input tensor: 0 indicates forward direction and
    1 indicates reverse direction. If omitted, all scan_input tensors will be scanned
