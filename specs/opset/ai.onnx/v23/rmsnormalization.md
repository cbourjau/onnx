...
type_constraints:
- name: T
  doc: Constrain input X type to float tensors.
  allowed:
  - tensor(bfloat16)
  - tensor(double)
  - tensor(float)
  - tensor(float16)
- name: V
  doc: Constrain output Y and scale type to float tensors.
  allowed:
  - tensor(bfloat16)
  - tensor(double)
  - tensor(float)
...
- name: X
  type: T
  doc: The input tensor to be normalized. In general, the shape is (D1, D2, ... ,
    Dn) for n-dimensional data, where the root mean squared norm is taken over the
    last D dimensions, D is determined by the axis attribute.
- name: scale
  type: V
  doc: Scale tensor. Scale tensor shape should be broadcastable to the normalized
    shape.
outputs:
- name: Y
  type: V
  doc: Output data tensor. Same shape as X
attributes:
- name: axis
  type: int
  doc: The first normalization dimension. If rank(X) is r, axis' allowed range is
    [-r, r). Negative value means counting dimensions from the back.
  required: false
  default: -1
- name: epsilon
  type: float
