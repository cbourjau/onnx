...
- name: X
  type: T
  doc: The input tensor representing the token embeddings. 4D tensor with shape `(batch_size,
    num_heads, sequence_length, head_size)` or 3D tensor with shape `(batch_size,
    sequence_length, hidden_size)`. For cases with a 4D input tensor, `head_size`
    has to be even. For cases with a 3D input tensor, `num_heads` attribute must be
    provided and `hidden_size` must be an even multiple of `num_heads` where `hidden_size
    = num_heads * head_size`
- name: cos_cache
  type: T
  doc: The cosine values for the rotation. 2D tensor with shape `(max_position_id_plus_1,
    head_size / 2)` for full rotation or `(max_position_id_plus_1, rotary_embedding_dim
    / 2)` for partial rotation when `position_ids` are provided. 3D tensor with shape
    `(batch_size, sequence_length, head_size / 2)` for full rotation or `(batch_size,
    sequence_length, rotary_embedding_dim / 2)` for partial rotation when `position_ids`
    are not provided. `max_position_id_plus_1` is a parameter to the model.
- name: sin_cache
  type: T
  doc: The sine values for the rotation. 2D tensor with shape `(max_position_id_plus_1,
    head_size / 2)` for full rotation or `(max_position_id_plus_1, rotary_embedding_dim
    / 2)` for partial rotation when `position_ids` are provided. 3D tensor with shape
    `(batch_size, sequence_length, head_size / 2)` for full rotation or `(batch_size,
    sequence_length, rotary_embedding_dim / 2)` for partial rotation when `position_ids`
...
- name: position_ids
  type: M
  doc: The position indices for the tokens. 2D tensor with shape `(batch_size, sequence_length)`
  kind: Option
outputs:
- name: Y
  type: T
  doc: Tensor with same shape as input.
attributes:
- name: interleaved
  type: int
  doc: Rotate using interleaved pattern. Default value is 0 (False).
  required: false
  default: 0
- name: num_heads
  type: int
  doc: 'Number of attention heads. Must be provided when input is a 3D tensor. '
  required: false
- name: rotary_embedding_dim
  type: int
  doc: Rotary embedding dimension used to apply partial rotary embeddings.
  required: false
  default: 0
---
