...
- name: past_cache
  type: T
  doc: Past state cache for key or value with shape `(batch_size, D1, D2, ..., max_sequence_length,
    ..., Dn)`.
  differentiable: true
- name: update
  type: T
  doc: New update tensor with shape `(batch_size, D1, D2, ..., sequence_length, ...,
    Dn)`.
  differentiable: true
- name: write_indices
  type: tensor(int64)
  doc: Write indices for the incoming update tensor in the cache. Shape is `(batch_size,)`.
...
- name: present_cache
  type: T
  doc: Updated cache. Same shape as `past_cache`.
  differentiable: true
attributes:
- name: axis
  type: int
  doc: Sequence dimension of the `past_cache` and `update` tensors. It cannot be 0
    (the batch dimension). Default is -2.
  required: false
  default: -2
- name: mode
  type: string
  doc: Write mode of cache update. Supported modes include `linear` and `circular`.
