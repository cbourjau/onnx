...
- name: input
  type: T
  doc: Input tensor of shape (N, C) or (N, C, d1, d2, ..., dk).
- name: target
  type: Tind
  doc: Target tensor of shape (N) or (N, d1, d2, ..., dk). Target element value shall
    be in range of [0, C). If ignore_index is specified, it may have a value outside
    [0, C) and the target values should either be in the range [0, C) or have the
    value ignore_index.
- name: weight
  type: T
  doc: Optional rescaling weight tensor. If given, it has to be a tensor of size C.
...
- name: reduction
  type: string
  doc: 'Type of reduction to apply to loss: none, sum, mean (default). ''none'': the
    output is the loss for each sample. ''sum'': the output will be summed. ''mean'':
