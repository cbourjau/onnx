---
inherit: v13/quantizelinear
since_version: 19
type_constraints:
  update:
  - name: T1
    doc: Constrain 'x' to float, float16, bfloat16 or int32 tensor.
    allowed:
    - tensor(bfloat16)
    - tensor(float)
    - tensor(float16)
    - tensor(int32)
  - name: T2
    doc: Constrain 'y_zero_point' and 'y' to 8-bit integer/float tensor.
    allowed:
    - tensor(float8e4m3fn)
...
attributes:
  ...
  - name: saturate
    ...
    doc: The parameter defines how the conversion behaves if an input value is out
      of range of the destination type. It only applies for float 8 quantization (float8e4m3fn,
