...
- name: y
  type: T
  doc: Output tensor of the same type as 'x' with cumulative products of the x's elements
  differentiable: true
attributes:
- name: exclusive
  type: int
  doc: If set to 1 will return exclusive product in which the top element is not included.
    In other terms, if set to 1, the j-th output element would be the product of the
    first (j-1) elements. Otherwise, it would be the product of the first j elements.
  required: false
  default: 0
- name: reverse
  type: int
  doc: If set to 1 will perform the products in reverse direction.
  required: false
  default: 0
---
