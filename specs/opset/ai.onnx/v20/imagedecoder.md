...
type_constraints:
- name: T1
  doc: Constrain input types to 8-bit unsigned integer tensor.
  allowed:
  - tensor(uint8)
- name: T2
  doc: Constrain output types to 8-bit unsigned integer tensor.
  allowed:
  - tensor(uint8)
inputs:
- name: encoded_stream
...
- name: pixel_format
  type: string
  doc: Pixel format. Can be one of "RGB", "BGR", or "Grayscale".
  required: false
  default: RGB
---
