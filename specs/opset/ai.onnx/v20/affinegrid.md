...
- name: T1
  ...
  - tensor(float16)
- name: T2
  doc: Constrain size's type to int64 tensors.
  allowed:
  - tensor(int64)
inputs:
- name: theta
  type: T1
  doc: input batch of affine matrices with shape (N, 2, 3) for 2D or (N, 3, 4) for
    3D
  differentiable: false
- name: size
  type: T2
  doc: the target output image size (N, C, H, W) for 2D or (N, C, D, H, W) for 3D
  differentiable: false
outputs:
- name: grid
  type: T1
  doc: output tensor of shape (N, H, W, 2) of 2D sample coordinates or (N, D, H, W,
    3) of 3D sample coordinates.
  differentiable: true
attributes:
- name: align_corners
  type: int
  doc: if align_corners=1, consider -1 and 1 to refer to the centers of the corner
    pixels. if align_corners=0, consider -1 and 1 to refer to the outer edge the corner
    pixels.
  required: false
  default: 0
---
