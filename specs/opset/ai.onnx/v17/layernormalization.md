...
type_constraints:
- name: T
  doc: Constrain input types and output Y type to float tensors.
  allowed:
  - tensor(bfloat16)
  - tensor(double)
  - tensor(float)
  - tensor(float16)
- name: U
  doc: Type of Mean and InvStdDev tensors.
  allowed:
  - tensor(bfloat16)
  - tensor(float)
inputs:
...
- name: InvStdDev
  type: U
  doc: Saved inverse standard deviation used during training to speed up gradient
    computation.
  kind: Option
attributes:
- name: axis
...
- name: stash_type
  type: int
  doc: Type of Mean and InvStdDev. This also specifies stage one's computation precision.
  required: false
  default: 1
---
