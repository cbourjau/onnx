...
function_body: |-
  <
    ...
    opset_import: ["" : 17]
  >
  HannWindow <output_datatype,periodic>(size) => (output)
  {
...
type_constraints:
- name: T1
  doc: Constrain the input size to int32_t or int64_t.
  allowed:
  - tensor(int32)
  - tensor(int64)
- name: T2
  doc: Constrain output types to numeric tensors.
  allowed:
  - tensor(bfloat16)
  - tensor(double)
  - tensor(float)
...
- name: size
  type: T1
  doc: A scalar value indicating the length of the window.
  differentiable: false
outputs:
- name: output
  type: T2
  doc: 'A Hann window with length: size. The output has the shape: [size].'
  differentiable: false
attributes:
...
- name: periodic
  type: int
  doc: 'If 1, returns a window to be used as periodic function. If 0, return a symmetric
