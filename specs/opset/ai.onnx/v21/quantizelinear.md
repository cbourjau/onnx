---
inherit: v19/quantizelinear
since_version: 21
type_constraints:
  update:
  - name: T1
    doc: The type of the input 'x'.
    allowed:
    - tensor(bfloat16)
    - tensor(float)
    - tensor(float16)
...
attributes:
  ...
  - name: output_dtype
    ...
    doc: (Optional) The output data type. If not supplied, the output data type is
      inferred from `y_zero_point` data type (`T2`). If neither `output_dtype` nor
      `y_zero_point` are supplied, output data type is uint8. If both `output_dtype`
      and `y_zero_point` are specified, `output_dtype` must be `T2`.
    required: false
    default: 0
  update:
  - name: axis
