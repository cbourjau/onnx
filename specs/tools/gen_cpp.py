...
import jinja2
from build_docs import SPECS_DIR, collect_ops
from models import (
    Attribute,
    BaseSpec,
    TypeConstraint,
...
_DIFF_ENUM_MAP = {
    ...
    "differentiable": "OpSchema::Differentiable",
    "non_differentiable": "OpSchema::NonDifferentiable",
}
...
class CppVariable:
    ...
    def diff_enum(self) -> str:
        if self._var.differentiation_category not in _DIFF_ENUM_MAP:
            raise ValueError(
                f"Unknown differentiation category: {self._var.differentiation_category!r}"
            )
        return _DIFF_ENUM_MAP[self._var.differentiation_category]
